...
def get_avg_mins_pg(df, windows=[1, 2, 3, 4, 5]):
    """Generate avg points in previous x games feature"""
    df = df.sort_values(by=['name', 'season', 'gameweek'])
    for window in windows:
        df['avg_minutes_last_{window}'] = df.groupby('name')['minutes'].apply(lambda x: x.shift(1).rolling(window=window, min_periods=1).mean())
    return df
...
def generate_rolling_form(df, form_over=[3,5,10]):
    ...
    for window in form_over:
        df['avg_points_last_{window}'] = df.groupby('name')['total_points'].apply(lambda x: x.shift(1).rolling(window=window, min_periods=1).mean())
    return df
